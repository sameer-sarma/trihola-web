...
const PublicProfilePage: React.FC = () => {
  ...
  const relationshipStrip = useMemo(() => {
    if (isOwnProfile) return null;
    ...
    const badge = relationshipBadgeLabel(relationship);
    const preview = relationship.commonContactsPreview ?? [];
    ...
    return (
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
          <div style={{ fontWeight: 800, whiteSpace: "nowrap" }}>Relationship</div>
          ...
          <div className="th-text-muted" style={{ marginLeft: 6, whiteSpace: "nowrap" }}>
            {badge}
            {relationship.commonContactsCount > 0
              ? ` • ${relationship.commonContactsCount} common`
              : ""}
          </div>
        </div>

        <div className="th-text-muted" style={{ fontWeight: 800 }}>
          →
        </div>
      </button>
    );
  }, [isOwnProfile, relationship, relationshipLoading]);

  const bizStrip = useMemo(() => {
    if (bizLoading) {
      return (
        <div className="th-text-muted" style={{ marginTop: 10 }}>
          Loading businesses…
    ...
    if (bizError) {
      return (
        <div className="th-text-muted" style={{ marginTop: 10 }}>
          Could not load businesses: {bizError}
        </div>
      );
    }
    ...
    const shown = businesses.slice(0, 6);
    ...
    return (
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
          <div style={{ fontWeight: 800, whiteSpace: "nowrap" }}>Businesses</div>
          ...
          <div className="th-text-muted" style={{ marginLeft: 6, whiteSpace: "nowrap" }}>
            {businesses.length} member{businesses.length === 1 ? "" : "s"}
          </div>
        </div>
  ...
  }, [bizLoading, bizError, businesses, isOwnProfile]);
                    ...
                    >
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        <div className="pp-strip-avatars">
                          {(relationship.commonContactsPreview ?? []).slice(0, 3).map((c, idx) => {
                            const name =
                              [c.firstName, c.lastName].filter(Boolean).join(" ").trim() ||
                              c.slug ||
                              "Contact";
