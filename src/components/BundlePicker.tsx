...
const BundlePicker: React.FC<Props> = ({ value, onChange, fetchItems, placeholder="Search bundle…", disabled }) => {
  ...
  const choose = (it: PickerItem) => { setSelected(it); onChange(it.id, it); setOpen(false); };
  const clear = () => { setSelected(undefined); onChange(null); };
    ...
    <div className="select-popover">
      ...
      {open && (
        <div className="select-popover-panel" onMouseDown={(e)=>e.preventDefault()}>
          ...
          {!busy && items.length === 0 && <div className="select-empty">No bundles</div>}
          {!busy && items.map(it => (
